...
class ReutersSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        for r, t in zip(result, timestamp):
            ...
            item = {'url': article_url, 'title': article_title,
                    'publish_date': t, 'source': 'reuters',
                    'original_key': ent}
            parse_dyn = functools.partial(self.parse_article, item=item)
            yield scrapy.Request(article_url, callback=parse_dyn)

    def parse_article(self, response, item):
        docs = response.css('.StandardArticleBody_body').xpath(
            '//p/text()').extract()
        item['article'] = '\n'.join(docs)
        return item
